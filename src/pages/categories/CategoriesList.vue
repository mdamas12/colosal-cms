      ...
      <div class="col-md-12 col-xs-12 q-pt-lg">
        <div class="row col-xs-12 q-pb-xl">
          <div class="col">
            ...
            <small class="text-subtitle2 text-grey-6 q-mb-none" >Tienes 5 categorias</small>
            <!--<q-skeleton v-else type="text" width="50%" animation="fade" />-->
          </div>
          <div class="col">
              <q-btn color="primary" label="Create category" class="float-right" @click="$router.push({ name : 'CategoriesCreate'/* , params : {contact : contact }  */})"/>
          </div>
        </div>
        ...
        <q-card class="shadow-6" style="margin-top:20px">
          <q-card-section>
            <div class="q-pa-md">
              <q-list>
                <q-item clickable v-ripple @click="$router.push({ name : 'CategoriesDetail' , params : {category : category }  })" v-for="category in categories">
...
export default Vue.extend({
  ...
  mounted () {
    ...
    let subscription = CategoriesService.getCategories().subscribe( {
        next: (data: Category[]) => {
          vm.categories = data
        },
